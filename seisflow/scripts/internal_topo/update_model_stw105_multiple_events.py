...
def kernel(lat_range, lon_range, kernel_path,
    ...
    input_depth_0_360 = np.zeros_like(lon2)
    raw_depth = np.loadtxt(model_path, skiprows=1)
    icount = 0
    for ilat in range(len(lat1)):
        for ilon in range(len(lon1)):
            input_depth_0_360[ilon, ilat] = raw_depth[icount, 3]
            icount += 1
    # reshape to the shape from -180 to 180
    input_depth = np.zeros_like(input_depth_0_360)
    input_depth[901:, :] = input_depth_0_360[:900, :]
    input_depth[:901, :] = input_depth_0_360[900:, :]
    input_depth = input_depth+650

    # * read in the model and kernel
    kernel_path_list = kernel_path.split(",")
    input_kernel_all = np.zeros_like(lon2)
    for each_kernel_path in kernel_path_list:
        raw_data = np.loadtxt(each_kernel_path)
        # * interp
