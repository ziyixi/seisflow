...
from .update_model_stw105_multiple_events import kernel as update_model_stw105_kernel
...
def kernel(windows_path, data_asdf_path, sync_asdf_path_raw, sync_asdf_path_perturbed, data_info_directory, stations_path,
    ...
    # * need to split: windows_path data_asdf_path sync_asdf_path_raw sync_asdf_path_perturbed
    # * kernel path will be used in multiple kernel updating scripts
    # * prepare to used parameters
    windows_list = [load_pickle(each_windows_path)
                    for each_windows_path in windows_path.split(",")]
    consider_surface = False
    data_virasdf_body_list = []
    sync_virasdf_body_raw_list = []
    sync_virasdf_body_perturbed_list = []
    for each_data_asdf_path, each_sync_asdf_path_raw, each_sync_asdf_path_perturbed in zip(data_asdf_path.split(","), sync_asdf_path_raw.split(","), sync_asdf_path_perturbed.split(",")):
        data_virasdf_body_list.append(VirAsdf())
