...
def get_script(windows_directory, output_directory, time_length, station_fname, min_periods, max_periods, data_asdf_directory, sync_asdf_directory, data_info_directory,
    ...
    if (len(listdir(data_info_directory)) == 0):
        ...
        result += f"srun -n {n_node*n_cores_each_node} {py} -m seisflow.scripts.shared.mpi_extract_data_info --asdf_directory {data_asdf_directory} --station_fname {station_fname} --output_dir {data_info_directory} ;"
    else:
        pass
    # get windows
    result += f"srun -n {n_node*n_cores_each_node} {py} -m seisflow.scripts.shared.mpi_tao_2018_ggg_windows --data_info_directory {data_info_directory} --time_length {time_length} --output_dir {windows_directory} ;"
    # run mpi_calculate_misfit_windows.py
    result += f"srun -n {n_node*n_cores_each_node} {py} -m seisflow.scripts.shared.mpi_calculate_misfit_windows --windows_directory {windows_directory} --output_directory {output_directory} --min_periods {min_periods} --max_periods {max_periods} --data_asdf_directory {data_asdf_directory} --sync_asdf_directory {sync_asdf_directory} --data_info_directory {data_info_directory} ;"
    return result
...
if __name__ == "__main__":
    ...
    def main(windows_directory, output_directory, time_length, station_fname, min_periods, max_periods, data_asdf_directory, sync_asdf_directory, data_info_directory,
             n_node, n_cores_each_node, used_time):
        py = sys.executable
