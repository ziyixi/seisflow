...
def cp_stations_adjoint2structure(py, stations_adjoint_directory, base_directory):
    ...
    """
    script = f"ibrun -n 1 {py} -m seisflow.scripts.shared.cp_stations_adjoint2structure --stations_adjoint_directory {stations_adjoint_directory} --base_directory {base_directory}; \n"
    return script
...
def ln_adjoint_source_to_structure(py, adjoint_directoy, base_directory):
    ...
    """
    script = f"ibrun -n 1 {py} -m seisflow.scripts.shared.ln_adjoint2sem --adjoint_directoy {adjoint_directoy} --base_directory {base_directory}; \n"
    return script
...
def collect_sync_files(py, search_directoy, output_directory):
    ...
    """
    script = f"ibrun -n 1 {py} -m seisflow.scripts.shared.collect_sync_asdf --search_directoy {search_directoy} --output_directory {output_directory}; \n"
    return script
...
def convert_green2conv_processed(py, nproc, cmts_directory, green_directory, conv_directory, waveform_length, taper_tmin_tmaxs, periods, sampling_rate):
    ...
    """
    script = f"ibrun -n {nproc} {py} -m seisflow.scripts.source_inversion.mpi_convert_green2sync --cmts_directory {cmts_directory} --green_directory {green_directory} --conv_directory {conv_directory} --waveform_length {waveform_length} --taper_tmin_tmaxs {taper_tmin_tmaxs} --periods {periods} --sampling_rate {sampling_rate}; \n"
    return script
...
def change_simulation_type(py, base_directory, simulation_type):
    ...
    """
    script = f"ibrun -n 1 {py} -m seisflow.scripts.shared.change_simulation_type --base_directory {base_directory} --simulation_type {simulation_type}; \n"
    return script
...
def source_inversion_single_step(iter_number, py, n_total, n_each, n_iter, nproc, n_node, ntasks, partition, simulation_time_step1, account,
    ...
    result += calculate_adjoint_source_raw(py, n_total, iter_misfit_windows, stations, raw_sync, iter_conv1_processed,
                                           data_processed, iter_adjoint_source, body_periods, surface_periods)
