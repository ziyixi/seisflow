...
def main(base_dir, sem_bin_dir, output_dir, nproc, n_node, n_cores, partition, used_time, account, source_mask_km, receiver_mask_km):
    ...
    thecommand = ""
    thecommand += f"{pyexec} -m seisflow.scripts.structure_inversion.generate_source_mask_list --base_directory {base_dir} --source_mask_km {source_mask_km} --receiver_mask_km {receiver_mask_km};"
