"""
process_kernel_azi.py: submit a job to process kernel. Firstly do the kernel summation along with preconditioning, later smooth the kernel. In the last generate a new model with LINE_SEARCH_PERTURBATION perturbation for doing line search.
"""
import sys
...
def do_smoothing(kernel_process_directory, sigma_h, sigma_v, input_dir, output_dir, n_tasks):
    ...
    to_smooth_kernel_names = [
        "bulk_c_kernel", "bulk_betav_kernel", "bulk_betah_kernel", "eta_kernel", "Gc_prime_kernel", "Gs_prime_kernel"]
...
def iter1_generate_perturbed_kernel(kernel_process_directory, perturbed_value):
    ...
    current_path = str(sh.pwd())[:-1]  # pylint: disable=not-callable
    result += f"cd {kernel_process_directory};"
    # when we run add_model_globe_azi, INPUT_MODEL(from previous gll directory), INPUT_GRADIENT(from ln smooth), topo(from database) have all been established.
    result += f"ibrun ./bin/xadd_model_azi {perturbed_value};"
...
def itern_generate_perturbed_kernel(kernel_process_directory, perturbed_value, pyexec):
    ...
    result += f"cd {kernel_process_directory};"
    # when we run add_model_globe_azi, INPUT_MODEL(from previous gll directory), INPUT_GRADIENT(from ln smooth), topo(from database) have all been established.
    result += f"ibrun ./bin/xadd_model_azi_cg {perturbed_value};"
    # we should move the kernel files in OUTPUT_MODEL to perturbed_{perturbed_value}_for_line_search
    result += f"mkdir -p perturbed_{perturbed_value}_for_line_search;"
