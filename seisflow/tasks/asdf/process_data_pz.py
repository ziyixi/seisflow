...
def process_single_event(min_periods, max_periods, taper_tmin_tmax, asdf_filename, waveform_length, sampling_rate, output_directory, correct_cea, cea_correction_file, paz_path):
    ...
    for min_period, max_period in zip(min_periods, max_periods):
        ...
        def process_function(st, inv):
            ...
            st = remove_response_paz(
                st, paz_path, pre_filt)  # pylint: disable=cell-var-from-loop
            ...
            if "N" in components and "E" in components:
                ...
                return
            # bandpass filter
            st.filter("bandpass", freqmin=1.0/max_period,  # pylint: disable=cell-var-from-loop
                      freqmax=1.0/min_period, corners=2, zerophase=True)  # pylint: disable=cell-var-from-loop
