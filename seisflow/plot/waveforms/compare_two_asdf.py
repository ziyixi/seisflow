...
def build_to_plot_traces(obs_ds, syn_ds, trace_length, info_dir):
    ...
    # get keys
    key_obs = set(obs_ds.get_waveforms_list())
    key_syn = set(syn_ds.get_waveforms_list())
    ...
    for key in keys:
        ...
        info = info_dict[key]
        obs_st = obs_ds.get_waveforms()[key].copy()
        syn_st = syn_ds.get_waveforms()[key].copy()
...
@click.command()
@click.option('--obs_asdf', required=True, type=str, help="obs asdf path")
@click.option('--syn_asdf', required=True, type=str, help="syn asdf path")
@click.option('--azimuth_width', required=True, type=int, help="the azimuth width for each page")
@click.option('--output_pdf', required=True, type=str, help="the output pdf file path")
@click.option('--waves_perpage', required=True, type=int, help="the number of waves to plot each page")
@click.option('--trace_length', required=True, type=int, help="the trace length to plot")
@click.option('--info_dir', required=True, type=str, help="the data info directory")
@click.option('--windows_path', required=False, type=str, default="", help="the windows file path")
def main(obs_asdf, syn_asdf, azimuth_width, output_pdf, waves_perpage, trace_length, info_dir):
    obs_ds = VirAsdf()
    obs_ds.read_asdf(obs_asdf)
    syn_ds = VirAsdf()
    syn_ds.read_asdf(syn_asdf)
