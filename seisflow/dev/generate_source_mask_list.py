...
@click.option('--source_mask', required=True, type=float, help="radius for source mask in km")
@click.option('--receiver_mask', required=True, type=float, help="radius for receiver mask in km")
def main(base_directory, source_mask, receiver_mask):
    ...
    for each_event_dir in all_event_dirs:
        ...
        receiver_mask_append = np.ones(
            (receiver_mask.shape[0], 1)) * receiver_mask
        ...
        np.savetxt(output_path, mask_list)
